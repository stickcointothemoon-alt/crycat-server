...
function loadDB(){
  try{
    ...
    (db.users||[]).forEach(u => users.set(u.lower, { username:u.username, id:u.id, secret:u.secret }));
...
function saveDB(){
  ...
  setTimeout(()=>{
    ...
    try{
      ...
      const db = {
        users: [...users.entries()].map(([lower,u]) => ({ lower, username:u.username, id:u.id, secret:u.secret })),
...
const server = http.createServer(async (req,res)=>{
  ...
  if(req.url==='/api/score'){
    ...
    if(trueScore > prev){ scores.set(lower, trueScore); saveDB(); }
  ...
  if(req.url==='/api/leaderboard'){
    return send(res,200,{ top: topN(50) });
  }
